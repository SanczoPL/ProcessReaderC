...
void getOpt(int *argc, char *argv[], char *state, char * input_param[], char * file_name[]);
...
void redirectStdout(char * name)
{
	...
	if (-1 == dup2(outputFile, fileno(stdout))) { perror("cannot redirect stdout"); exit(-55); }
}
...
void stopRedirectStdout()
{
	...
	#endif
	fflush(stdout); close(outputFile);
